...
from user_profile.models import UserProfile,AutoDriver,AutoDriverLocation
...
def get_user(request):
    ...
    try:
        ...
        new=True
        usr=UserProfile.objects.create(mobile=str(number),name=str(name),)
...
def answer(request):
    ...
    response.say("Welcome to Namma Yatri ")
    with response.gather(
        num_digits=1,
        action=reverse('book-auto'),
        timeout=10,
    ) as g:
        g.say(message="To book an auto press 1, To repeat press 9, to exit press 0 ",)
